...
import { Activity, AlertTriangle, Barcode, Bell, Camera, Check, Download, Edit, Edit2, Package, Plus, Search, Trash2, TrendingUp, Upload, Warehouse, X } from 'lucide-react-native';
...
// ==========================================
// ESTILOS NOVOS — Tela "Produtos" (dark theme)
// ==========================================
...
export default function App() {
  ...
  const theme = useMemo(() => ({
    background: isDark ? '#041642' : '#F8FAFC',
    surface: isDark ? '#002063' : '#FFFFFF',
    surfaceAlt: isDark ? '#19192e' : '#E0E7FF',
    border: isDark ? 'rgba(25,25,46,0.8)' : 'rgba(0,32,99,0.15)',
    borderSoft: isDark ? 'rgba(25,25,46,0.4)' : 'rgba(0,32,99,0.05)',
    text: isDark ? '#F1F5F9' : '#041642',
    title: isDark ? '#FFFFFF' : '#002063',
    muted: isDark ? '#94A3B8' : '#003e91',
    subtle: isDark ? '#003e91' : '#3B82F6',
    inputBg: isDark ? '#041642' : '#FFFFFF',
    chipBg: isDark ? '#002063' : '#E0E7FF',
    chipText: isDark ? '#F1F5F9' : '#002063',
    eanBg: isDark ? '#19192e' : '#F1F5F9',
    actionBg: isDark ? '#003e91' : '#EEF2FF',
    closeBg: isDark ? '#041642' : '#F8FAFC',
    headerBg: isDark ? '#041642' : '#002063',
    headerPanelBg: isDark ? '#002063' : '#041642',
    headerPanelBorder: isDark ? 'rgba(25,25,46,0.6)' : 'rgba(255,255,255,0.2)',
    overlay: isDark ? 'rgba(4,22,66,0.85)' : 'rgba(0,32,99,0.6)',
    bottomOverlay: isDark ? 'rgba(4,22,66,0.75)' : 'rgba(0,32,99,0.4)',
    sidebarOverlay: isDark ? 'rgba(4,22,66,0.5)' : 'rgba(0,32,99,0.2)',
    cardBg: isDark ? '#002063' : '#FFFFFF',
  }), [isDark]);
  ...
  const [produtoDetalhe, setProdutoDetalhe] = useState<ProdutoComAnalise | null>(null);
  ...
  const lerPlanilhaComoLinhas = async (arquivo: DocumentPicker.DocumentPickerAsset) => {
    ...
    if (extensao === 'xlsx') {
      const conteudoBase64 = await FileSystem.readAsStringAsync(arquivo.uri, { encoding: FileSystem.EncodingType.Base64 });
      const workbook = XLSX.read(conteudoBase64, { type: 'base64' });
      const primeiraAba = workbook.SheetNames[0];
      if (!primeiraAba) return [];
      const worksheet = workbook.Sheets[primeiraAba];
      const linhas = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' }) as string[][];
      return linhas.map((linha) => linha.map((celula) => String(celula).trim())).filter((linha) => linha.some((coluna) => coluna));
    }

    const conteudoTexto = await FileSystem.readAsStringAsync(arquivo.uri);
  ...
  const exportarParaExcel = useCallback(async () => {
    ...
    try {
      ...
      const wsTodos = XLSX.utils.aoa_to_sheet([
      ...
      XLSX.utils.book_append_sheet(workbook, wsTodos, 'Produtos');

      const wsVencidos = XLSX.utils.aoa_to_sheet(montarLinhasProdutos(listExpiredProducts(produtosVisiveis)));
      XLSX.utils.book_append_sheet(workbook, wsVencidos, 'Vencidos');

      const wsProximos = XLSX.utils.aoa_to_sheet(montarLinhasProdutos(listNearExpiryProducts(produtosVisiveis, { days: 30 })));
      XLSX.utils.book_append_sheet(workbook, wsProximos, 'Proximos');

      const wsEstoqueBaixo = XLSX.utils.aoa_to_sheet(montarLinhasProdutos(listLowStockProducts(produtosVisiveis, { minStock: 5 })));
      XLSX.utils.book_append_sheet(workbook, wsEstoqueBaixo, 'Estoque_Baixo');

      const wsResumo = XLSX.utils.aoa_to_sheet(planilhaResumoColaborador);
      XLSX.utils.book_append_sheet(workbook, wsResumo, 'Resumo_Colaborador');

      const wsHistorico = XLSX.utils.aoa_to_sheet(planilhaHistorico);
  ...
  const baixarModeloPlanilha = useCallback(async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, wsModelo, 'Modelo_Completo');

      const wsMinimo = XLSX.utils.aoa_to_sheet([
        ['Nome', 'Codigo_EAN', 'Validade'],
        ['Paracetamol 750mg', '7890001112223', '2026-10-15'],
      ]);
      XLSX.utils.book_append_sheet(workbook, wsMinimo, 'Modelo_Minimo');
  ...
  const importarDeExcel = async () => {
    try {
      ...
      const inicioDados = linhas.findIndex((linha: string[]) => linhaPareceCabecalhoProdutos(linha.map((coluna) => String(coluna || ''))));
      const cabecalhoLinha = inicioDados >= 0 ? linhas[inicioDados] : [];
      const cabecalhos = cabecalhoLinha.map((coluna: string) => normalizarCabecalho(String(coluna || '')));
      const linhasDados = inicioDados >= 0 ? linhas.slice(inicioDados + 1) : linhas;

      for (let i = 0; i < linhasDados.length; i++) {
        const cols = linhasDados[i].map((coluna: string) => String(coluna || '').trim());
        if (cols.some(Boolean)) {
          const registro = cabecalhos.length
            ? cabecalhos.reduce<Record<string, string>>((acc: Record<string, string>, cabecalho: string, index: number) => {
  ...
  const importarBaseInternaEan = async () => {
    try {
      ...
      const cabecalhos = linhas[0].map((coluna: string) => normalizarCabecalho(coluna));
      ...
      for (let i = 1; i < linhas.length; i++) {
        const colunas = linhas[i].map((coluna: string) => String(coluna || '').trim());
        if (colunas.length < 2) continue;

        const registro = cabecalhos.reduce<Record<string, string>>((acc: Record<string, string>, cabecalho: string, index: number) => {
  ...
  const renderAcoesSwipeDireita = useCallback((produto: ProdutoComAnalise) => (
    <View style={[styles.swipeActionsWrap, { marginBottom: 8, marginRight: 16 }]}>
      <TouchableOpacity style={[styles.swipeActionBtn, { backgroundColor: isDark ? 'rgba(0, 62, 145, 0.2)' : '#E0E7FF', borderColor: isDark ? 'rgba(0, 62, 145, 0.5)' : '#A5B4FC' }]} onPress={() => iniciarEdicao(produto)}>
        <View style={[styles.swipeIconBubbleEdit, { backgroundColor: isDark ? 'rgba(0, 62, 145, 0.3)' : '#C7D2FE' }]}><Edit2 size={16} color={isDark ? '#E0E7FF' : '#003e91'} /></View>
        <Text style={[styles.swipeActionText, { color: isDark ? '#E0E7FF' : '#003e91' }]}>Editar</Text>
      </TouchableOpacity>
      <TouchableOpacity style={[styles.swipeActionBtn, { backgroundColor: isDark ? 'rgba(239, 68, 68, 0.2)' : '#FEE2E2', borderColor: isDark ? 'rgba(239, 68, 68, 0.5)' : '#FECACA' }]} onPress={() => removerProduto(produto.id)}>
        <View style={[styles.swipeIconBubbleDelete, { backgroundColor: isDark ? 'rgba(239, 68, 68, 0.3)' : '#FECACA' }]}><Trash2 size={16} color={isDark ? '#FECACA' : '#EF4444'} /></View>
        <Text style={[styles.swipeActionText, { color: isDark ? '#FECACA' : '#EF4444' }]}>Excluir</Text>
      </TouchableOpacity>
    </View>
  ), [iniciarEdicao, removerProduto, isDark]);

  // ─────────────────────────────────────────────────────────────
  // renderProdutoItem — novo card compacto dark (design "Produtos")
  // Mantém o Swipeable para editar/excluir via swipe
  // ─────────────────────────────────────────────────────────────
  const renderProdutoItem = useCallback(({ item: p }: { item: ProdutoComAnalise }) => {
    return (
      <Swipeable
        ...
        {/* Usa o novo componente reutilizável — sem alterar lógica */}
        <ProductListItem
          produto={p}
          onPress={() => setProdutoDetalhe(p)}
          isDark={isDark}
        />
      </Swipeable>
    );
  }, [iniciarEdicao, produtoSwipeado, renderAcoesSwipeDireita]);

  // ─────────────────────────────────────────────────────────────
  // renderListaHeader — novo design "Produtos"
  // Transformado em função para evitar problemas de avaliação de prodStyles
  // ─────────────────────────────────────────────────────────────
  const renderListaHeader = useCallback(() => (
    <>
      {/* ── KPI Cards horizontais (mantidos sem alteração) ── */}
      ...
      {/* ── Nova barra de busca estilo "Produtos" ── */}
      <View style={prodStyles.searchBarWrap}>
        <View style={[prodStyles.searchBar, { backgroundColor: theme.inputBg, borderColor: theme.border }]}>
          <Search size={16} color={theme.subtle} />
          <TextInput
            style={[prodStyles.searchInput, { color: theme.text }]}
            placeholder="Buscar por nome ou EAN..."
            placeholderTextColor={theme.subtle}
            value={termoBusca}
            onChangeText={setTermoBusca}
          />
          {/* Ícone de scanner (abre câmera) */}
          <TouchableOpacity onPress={() => { void acionarCamera(); }} style={[prodStyles.searchScanBtn, { backgroundColor: theme.surfaceAlt }]}>
            <Barcode size={20} color={theme.title} />
          </TouchableOpacity>
        </View>
      </View>

      {/* ── Tabs de filtro horizontais ── */}
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={prodStyles.tabsContainer}
        style={prodStyles.tabsScroll}>
        {([
          ...
          const isActive = filtroValidade === tab.key;
          return (
            <TouchableOpacity
              key={tab.key}
              style={[
                prodStyles.tab,
                { backgroundColor: isActive ? theme.muted : theme.chipBg, borderColor: isActive ? theme.muted : theme.borderSoft },
              ]}
              onPress={() => setFiltroValidade(tab.key)}>
              <Text
                style={[
                  prodStyles.tabText,
                  { color: isActive ? theme.background : theme.chipText },
                ]}>
                {tab.label}
              </Text>
            </TouchableOpacity>
      ...
      {/* ── Linha: N itens encontrados + botão Ordenar ── */}
      <View style={prodStyles.countSortRow}>
        <Text style={prodStyles.countText}>
          {produtosFiltrados.length} iten{produtosFiltrados.length !== 1 ? 's' : ''} encontrado{produtosFiltrados.length !== 1 ? 's' : ''}
        </Text>
        <View style={prodStyles.sortActions}>
          {filtrosAvancadosAtivos > 0 && (
            <TouchableOpacity
              style={prodStyles.advancedFilterBtn}
              onPress={() => setShowFiltrosAvancados(true)}>
              <Search size={13} color="#7C3AED" />
              <Text style={prodStyles.advancedFilterText}>Filtros ({filtrosAvancadosAtivos})</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity
            style={prodStyles.sortBtn}
            onPress={() =>
              setOrdenacaoLista((c) =>
                c === 'risk-first' ? 'name-asc' : c === 'name-asc' ? 'name-desc' : 'risk-first'
              )
            }>
            <TrendingUp size={13} color="#94A3B8" />
            <Text style={prodStyles.sortBtnText}>
              {ordenacaoLista === 'name-asc'
                ? 'A-Z'
                : ordenacaoLista === 'name-desc'
                  ? 'Z-A'
                  : 'Ordenar'}
            </Text>
          </TouchableOpacity>
      ...
      {(filtroValidade !== 'todos' || filtrosAvancadosAtivos > 0 || ordenacaoLista !== 'risk-first') && (
        <TouchableOpacity
          style={prodStyles.clearFiltersRow}
          onPress={() => {
          ...
          <Text style={prodStyles.clearFiltersText}>Limpar filtros</Text>
        </TouchableOpacity>
      )}
    </>
  ), [totalQtdAuditada, larguraKpi, isCompact, totalMedidoPrincipal, totaisMedidosSecundarios, produtosFiltrados.length, qtdMarkdown, qtdRiscoImediato, termoBusca, setTermoBusca, acionarCamera, filtroValidade, setFiltroValidade, filtrosAvancadosAtivos, ordenacaoLista, setOrdenacaoLista]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <Modal visible={!!produtoDetalhe} transparent={true} animationType="fade" onRequestClose={() => setProdutoDetalhe(null)}>
        <View style={[styles.overlayModal, { backgroundColor: theme.overlay }]}>
          <Pressable style={StyleSheet.absoluteFillObject} onPress={() => setProdutoDetalhe(null)} />
          <View style={[styles.dialogBox, { backgroundColor: isDark ? '#002063' : '#FFFFFF', elevation: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, padding: 24, borderRadius: 16 }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <View style={{ flex: 1 }}>
                <Text style={{ color: isDark ? '#F1F5F9' : '#041642', fontSize: 20, fontWeight: '700' }}>{produtoDetalhe?.nome || 'Produto Sem Nome'}</Text>
                <Text style={{ color: isDark ? '#94A3B8' : '#003e91', fontSize: 14, marginTop: 4 }}>EAN: {produtoDetalhe?.codigo}</Text>
              </View>
              <TouchableOpacity onPress={() => setProdutoDetalhe(null)} style={{ padding: 4 }}><X color={isDark ? '#F1F5F9' : '#041642'} /></TouchableOpacity>
            </View>

            <View style={{ gap: 12, marginBottom: 24 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: isDark ? '#94A3B8' : '#6B7280', fontSize: 14 }}>Apresentação</Text>
                <Text style={{ color: isDark ? '#F1F5F9' : '#111827', fontSize: 14, fontWeight: '600' }}>{produtoDetalhe?.apresentacao || '-'}</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: isDark ? '#94A3B8' : '#6B7280', fontSize: 14 }}>Quantidade</Text>
                <Text style={{ color: isDark ? '#F1F5F9' : '#111827', fontSize: 14, fontWeight: '600' }}>{produtoDetalhe?.qtd} {produtoDetalhe?.unidade_medida}</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: isDark ? '#94A3B8' : '#6B7280', fontSize: 14 }}>Validade</Text>
                <Text style={{ color: isDark ? '#F1F5F9' : '#111827', fontSize: 14, fontWeight: '600' }}>{produtoDetalhe?.validade ? formataDataBR(produtoDetalhe.validade) : '-'}</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: isDark ? '#94A3B8' : '#6B7280', fontSize: 14 }}>Status</Text>
                <Text style={{ color: isDark ? '#F1F5F9' : '#111827', fontSize: 14, fontWeight: '600' }}>{produtoDetalhe?.statusValidade?.label || '-'}</Text>
              </View>
              {produtoDetalhe?.observacao ? (
                <View style={{ marginTop: 8 }}>
                  <Text style={{ color: isDark ? '#94A3B8' : '#6B7280', fontSize: 14, marginBottom: 4 }}>Observação</Text>
                  <Text style={{ color: isDark ? '#F1F5F9' : '#111827', fontSize: 14 }}>{produtoDetalhe.observacao}</Text>
                </View>
              ) : null}
            </View>

            <TouchableOpacity
              style={{ backgroundColor: isDark ? '#003e91' : '#003e91', paddingVertical: 14, borderRadius: 12, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
              onPress={() => {
                const p = produtoDetalhe;
                setProdutoDetalhe(null);
                if (p) iniciarEdicao(p);
              }}>
              <Edit2 size={18} color="#FFFFFF" />
              <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '600' }}>Editar Produto</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
